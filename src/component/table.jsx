  ...
  return (
    <table className="w-full border">
      <thead className="sticky top-[-1px] bg-white border shadow-lg">
        <tr className="text-lg font-extrabold border">
          <td className="w-[15%] border p-2">User Id</td>
          <td className="w-[25%] border p-2">Status</td>
          <td className="w-[35%] border p-2">Time</td>
          <td className="w-[25%] border p-2">Error</td>
        </tr>
      </thead>
      <tbody>
        {data.map((z) => {
          ...
          return (
            <tr key={z._id}>
              ...
              <td className="w-[25%] border p-2">
                {z.status === "success" && (
                  <div className="bg-green-600 px-4 rounded-full inline text-xs text-white py-1">
                    Success
                  </div>
                )}
                {z.status === "failed" && (
                  <div className="bg-red-600 px-4 rounded-full inline text-xs text-white py-1">
                    Failed
                  </div>
                )}
              </td>
              <td className="w-[35%] border p-2">
                {`${pad(getDate)}-${pad(getMonth)}-${pad(getYear)} ${pad(
                  getHours
                )}:${pad(getMinutes)}`}
              </td>
              <td className="w-[25%] border p-2">{z.errorMessage || "NA"}</td>
            </tr>
          );
        })}
      </tbody>
